...
func TestCodexRefreshesRevokedUnexpiredToken(t *testing.T) {
	...
	cand := candidate{Key: "codex/gpt-test", Provider: provider{Type: "codex"}}
...
func TestCodexFailedRefreshDoesNotStormOrExposeSecrets(t *testing.T) {
	...
	for range 3 {
		res := tryModel(httptest.NewRequest("POST", "/", nil), config{firstByte: time.Second}, candidate{Key: "codex/gpt-test", Provider: provider{Type: "codex"}}, []byte(`{}`), false)
