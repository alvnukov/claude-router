	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, effort := range ClaudeEfforts {
				route := cfg.RouteFor(tc.model, effort)
				if route.Mode != "pool" || len(next.ModelPools[route.Pool]) != 1 || next.ModelPools[route.Pool][0].Model != "p/"+tc.model {
					t.Fatalf("%s/%s lost local route: %+v", tc.model, effort, route)
				}
			}
		})
	}
...
func TestLegacyMigrationKeepsFailoverForConfiguredModelID(t *testing.T) {
	...
	cfg := Config{Local: next}
	if route := cfg.RouteFor("qwen3", "default"); route.Mode != "pool" || len(next.ModelPools[route.Pool]) == 0 {
		t.Fatalf("configured model lost its route: %+v", route)
	}
